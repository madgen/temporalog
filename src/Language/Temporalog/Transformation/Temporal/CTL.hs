...
round1 = transformBody (cata alg)
  ...
  alg (SAXF span child) = SNeg span $ SEX span $ SNeg span child
  alg (SAGF span child) = SNeg span $ SEF span $ SNeg span child
  alg (SAFF span child) = SNeg span $ SEG span $ SNeg span child
  alg (SAUF span child1 child2) =
    SConj span (SEU span (SNeg span child2)
                         (SConj span (SNeg span child1) (SNeg span child2)))
               (SNeg span $ SEG span $ SNeg span child2)
  alg s = embed s
...
round2 = transformBody (cata alg)
  ...
  alg :: Algebra (Base (Subgoal (BOp 'Temporal) Term))
                 (Subgoal (BOp 'Temporal) Term)
  alg (SEFF span child) = SEU span (SDogru span) child
  alg s = embed s
